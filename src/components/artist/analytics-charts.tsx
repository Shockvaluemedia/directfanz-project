...
export function EarningsChart({ data, period }: EarningsChartProps) {
  ...
  useEffect(() => {
    ...
    const trend =
      earnings.length > 1
        ? earnings[earnings.length - 1] > earnings[0]
          ? 'up'
          : 'down'
        : 'stable';
    ...
    <Card>
      <CardHeader className='flex flex-row items-center justify-between pb-2'>
        ...
        <div className='flex items-center'>
          ...
          <span className='text-sm font-medium'>Total: ${totalEarnings.toFixed(2)}</span>
        </div>
      </CardHeader>
      <CardContent>
        <div className='h-64'>
...
export function SubscriberChart({ data, period }: SubscriberChartProps) {
  ...
  const netGrowth = data?.length ? data[data.length - 1].subscribers - data[0].subscribers : 0;
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className='flex justify-between mt-4 text-sm text-muted-foreground'>
          ...
          <span>
            Retention: {totalNew > 0 ? Math.round((1 - totalCanceled / totalNew) * 100) : 0}%
          </span>
        </div>
      </CardContent>
    </Card>
...
export function ChurnRateChart({
  ...
  data: { tierId: string; tierName: string; churnRate: number }[];
}) {
  const chartRef = useRef<HTMLCanvasElement>(null);
  const chartInstance = useRef<Chart | null>(null);
