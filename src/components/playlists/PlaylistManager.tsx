  ...
  const toggleLike = async (playlistId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPlaylists(prev =>
          prev.map(p => (p.id === playlistId ? { ...p, hasLiked: liked, likeCount } : p))
        );
      }
    } catch (error) {
  ...
  const removeFromPlaylist = async (playlistId: string, itemId: string) => {
    try {
      ...
      if (response.ok) {
        setPlaylists(prev =>
          prev.map(p =>
            p.id === playlistId ? { ...p, items: p.items.filter(item => item.id !== itemId) } : p
          )
        );
            ...
            <DialogTitle className='flex items-center space-x-3'>
              ...
              <div>
                ...
                <p className='text-sm text-muted-foreground'>
                  ...
                  {selectedPlaylist && getTotalDuration(selectedPlaylist)}
                </p>
              </div>
            </DialogTitle>
          </DialogHeader>
