...
export default function CampaignCard({ campaign, showActions = true, compact = false }: CampaignCardProps) {
  const canParticipate = campaign.status === 'ACTIVE' && !campaign.isParticipating;
  const isActive = campaign.status === 'ACTIVE';

  return (
    <Card className={`hover:shadow-md transition-shadow duration-200 ${compact ? 'h-auto' : 'h-full'}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              {campaign.artist && (
                <p className="text-sm text-gray-600 truncate">
                  by {campaign.artist.displayName}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center justify-between bg-blue-50 rounded-lg px-3 py-2">
              ...
              {campaign.userSubmissionCount !== undefined && campaign.userSubmissionCount > 0 && (
                <span className="text-xs text-blue-600">
                  {campaign.userSubmissionCount} submission{campaign.userSubmissionCount !== 1 ? 's' : ''}
                </span>
              )}
            </div>
          )}
            ...
            <div className="flex gap-2">
              ...
              {canParticipate && (
                <Button
                  ...
                  asChild
                >
                  <Link href={`/campaigns/${campaign.id}?action=join`}>
                    Join Campaign
                  </Link>
                </Button>
              )}
              ...
              {campaign.isParticipating && isActive && (
                <Button
                  ...
                  asChild
                >
                  <Link href={`/campaigns/${campaign.id}?action=submit`}>
                    Submit
                  </Link>
                </Button>
              )}
