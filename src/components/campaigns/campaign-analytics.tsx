...
const EngagementMetrics = ({ campaign }: { campaign: Campaign }) => {
  ...
  const metrics = [
    { label: 'Daily Active Participants', value: Math.floor((campaign.participantCount || 0) * 0.3), change: '+12%' },
    { label: 'Avg. Submissions per User', value: (campaign.submissionCount || 0) / (campaign.participantCount || 1), change: '+5%' },
    { label: 'Social Shares', value: Math.floor((campaign.participantCount || 0) * 0.8), change: '+18%' },
    { label: 'Conversion Rate', value: campaign.completionRate || 0, change: '-2%', isPercentage: true }
  ];
        ...
        <CardContent>
          {campaign.participantCount && campaign.participantCount > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              <p>No activity yet</p>
              <p className="text-sm">Activity will appear once the campaign starts receiving participants</p>
            </div>
          )}
        </CardContent>
      </Card>
