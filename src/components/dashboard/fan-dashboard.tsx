...
export default function FanDashboard() {
  ...
  // Fetch fan stats
  const { data: statsResponse, loading: loadingStats } = useApi<{ data: { stats: FanStats } }>(
    '/api/fan/stats',
    {
      immediate: true,
    }
  ...
  const { data: subscriptionsResponse, loading: loadingSubscriptions } = useApi<{
    subscriptions: Subscription[];
  }>('/api/fan/subscriptions?status=active', {
    immediate: true,
  });

  // Fetch recent content
  const { data: recentContentResponse, loading: loadingRecentContent } = useApi<{
    content: RecentContent[];
  }>('/api/fan/content/recent?limit=6', {
    immediate: true,
  });

  // Fetch recommended artists
  const { data: recommendationsResponse, loading: loadingRecommendations } = useApi<{
    artists: RecommendedArtist[];
  }>('/api/recommendations/artists?limit=3', {
    immediate: true,
  });

  // Fetch participating campaigns
  const { data: campaignsResponse, loading: loadingCampaigns } = useApi<{
  ...
  const { data: submissionsResponse, loading: loadingSubmissions } = useApi<{
    submissions: CampaignSubmission[];
  }>('/api/fan/submissions?limit=5', {
    immediate: true,
  });
    ...
    <div className='space-y-8'>
      ...
      <div className='bg-gradient-to-r from-pink-500 to-indigo-600 rounded-xl p-8 text-white'>
        ...
        <p className='text-pink-100 text-lg'>
          Join artist campaigns, discover amazing content from your favorite artists and explore new
          ones.
        </p>
      </div>
                      ...
                      <div className='space-y-3'>
                        <div>
                          <div className='flex justify-between text-xs mb-1'>
                            <span>Progress</span>
                            <span>
                              {campaign.progress.current}/{campaign.progress.target}
                            </span>
                          </div>
                          <Progress
        ...
        <div className='space-y-8'>
          ...
          <div>
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6'>
              <LoadingState loading={loadingSubscriptions} fallback={<SkeletonCard lines={3} />}>
                {subscriptions.length > 0 ? (
                    ...
                    {subscriptions.slice(0, 3).map(sub => (
                      <div key={sub.id} className='flex items-center justify-between'>
                        <div className='flex items-center space-x-3'>
                          ...
                          <div>
                            <p className='font-medium text-gray-900'>{sub.artist.displayName}</p>
                            <p className='text-sm text-gray-600'>${sub.tier.price}/month</p>
                          </div>
                        </div>
                  ...
                  <div className='text-center py-4'>
                    ...
                    <p className='text-sm text-gray-400'>Support your favorite artists</p>
                  </div>
                )}
              </LoadingState>
            </div>
          ...
          <div>
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6'>
              <LoadingState loading={loadingRecommendations} fallback={<SkeletonCard lines={4} />}>
