...
export function ContentEditor({ content, onSave, onCancel }: ContentEditorProps) {
  ...
  const handleTierChange = (tierId: string, checked: boolean) => {
    setFormData(prev => ({
      ...prev,
      tierIds: checked ? [...prev.tierIds, tierId] : prev.tierIds.filter(id => id !== tierId),
    }));
  };
          ...
          <div className='flex items-start space-x-4'>
            ...
            <div className='flex-grow'>
              <div className='flex items-center space-x-2 mb-2'>
                ...
                <Badge variant='secondary'>{content.totalViews} views</Badge>
              </div>
              ...
              <p className='text-sm text-muted-foreground'>
                Last viewed{' '}
                {content.lastViewedAt
                  ? new Date(content.lastViewedAt).toLocaleDateString()
                  : 'Never'}
              </p>
            </div>
          </div>
      ...
      <div className='space-y-4'>
        ...
        <Select value={formData.visibility} onValueChange={handleVisibilityChange}>
          ...
          <SelectContent>
            ...
            <SelectItem value='TIER_LOCKED'>Tier Locked - Only for subscribers</SelectItem>
