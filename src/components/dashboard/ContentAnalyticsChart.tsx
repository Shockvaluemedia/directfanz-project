...
export function ContentAnalyticsChart({ data, topContent }: ContentAnalyticsChartProps) {
  ...
  const growthRate =
    previous7DaysViews > 0 ? ((last7DaysViews - previous7DaysViews) / previous7DaysViews) * 100 : 0;
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
              ...
              {topContent.slice(0, 10).map((content, index) => (
                <div key={content.id} className='flex items-center space-x-4'>
                  ...
                  <div className='flex-shrink-0 text-right'>
                    ...
                    <p className='text-xs text-muted-foreground'>{content.uniqueViews} unique</p>
                  </div>
                </div>
              ))}
        ...
        <Card>
          ...
          <CardContent>
            <div className='space-y-4'>
              {(() => {
                const typeStats = topContent.reduce(
                  (acc, content) => {
                    if (!acc[content.type]) {
                      acc[content.type] = {
                        totalViews: 0,
          ...
          <CardContent>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              <div>
                ...
                {(() => {
                  const typeAvg = topContent.reduce(
                    (acc, content) => {
                      if (!acc[content.type]) {
              ...
              <div>
                ...
                {(() => {
                  ...
                  return mostConsistent ? (
                    <div className='flex items-center space-x-2'>
                      <span className='font-medium truncate max-w-32'>{mostConsistent.title}</span>
                      <span className='text-sm text-muted-foreground'>
                        {((mostConsistent.uniqueViews / mostConsistent.totalViews) * 100).toFixed(
                          1
                        )}
                        % unique views
                      </span>
