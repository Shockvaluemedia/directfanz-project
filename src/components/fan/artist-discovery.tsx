  ...
  return (
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
      ...
      <div className='mb-8'>
        ...
        <p className='text-gray-600'>Find and support your favorite independent artists</p>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8'>
        {artists.map(artist => (
          ...
          >
            ...
            {/* Artist Info */}
            <div className='p-4'>
              <h3 className='text-lg font-semibold text-gray-900 mb-2'>{artist.displayName}</h3>
              ...
              <div className='flex justify-between items-center mb-3 text-sm text-gray-500'>
                ...
                <span>
                  {artist.tiers.length} tier{artist.tiers.length !== 1 ? 's' : ''}
                </span>
              </div>
              ...
              {artist.tiers.length > 0 && (
                <div className='space-y-2'>
                  <div className='text-sm font-medium text-gray-700'>Starting from:</div>
                  <div className='bg-gray-50 rounded-lg p-3'>
                    <div className='flex justify-between items-center'>
                      <span className='font-medium text-gray-900'>{artist.tiers[0].name}</span>
                      <span className='text-green-600 font-semibold'>
                        {formatPrice(artist.tiers[0].minimumPrice)}+/month
                      </span>
                    </div>
                    <p className='text-xs text-gray-600 mt-1'>{artist.tiers[0].description}</p>
                  </div>
                </div>
              )}
              ...
              {artist.content.length > 0 && (
                <div className='mt-3 pt-3 border-t border-gray-200'>
                  <div className='text-sm font-medium text-gray-700 mb-2'>Recent Content:</div>
                  <div className='flex gap-2'>
                    {artist.content.slice(0, 3).map(content => (
                      <div key={content.id} className='flex-1 bg-gray-100 rounded p-2 text-xs'>
                        <div className='font-medium text-gray-800 truncate'>{content.title}</div>
                        <div className='text-gray-500 capitalize'>{content.type.toLowerCase()}</div>
                      </div>
                    ))}
                  </div>
                </div>
