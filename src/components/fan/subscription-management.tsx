...
export default function SubscriptionManagement() {
  ...
  const handleCancelSubscription = async (subscriptionId: string) => {
    ...
    if (!confirm(`Are you sure you want to cancel your subscription to ${subscription.tier.artist.displayName}? You'll lose access to their exclusive content.`)) {
      return
    }
                ...
                {subscription.status === 'CANCELED' && (
                  <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded">
                    <p className="text-sm text-red-700">
                      This subscription has been canceled. You'll continue to have access until {formatDate(subscription.currentPeriodEnd)}.
                    </p>
                  </div>
                )}
              </div>
