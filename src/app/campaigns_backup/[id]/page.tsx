...
export default function CampaignDetailsPage() {
    ...
    try {
      ...
      const transformedCampaign: Campaign = {
        ...
        currentMetrics: {
          ...
          submissions: data.challenges?.reduce((sum: number, challenge: any) => sum + (challenge.submissionCount || 0), 0) || 0
        },
        artist: {
          id: data.artist.id,
        ...
        rewards: data.rewards?.map((reward: any) => 
          `${reward.title}: ${reward.description || 'Exclusive prize'}`
        ) || [
  ...
  const handleSubmission = async () => {
    try {
      ...
      setCampaign(prev => prev ? {
        ...prev,
        userSubmissions: [...prev.userSubmissions, {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'MUSIC_PROMOTION': return <Music className="w-4 h-4" />;
      case 'CONTEST': return <Trophy className="w-4 h-4" />;
      case 'COLLABORATION': return <Users className="w-4 h-4" />;
      case 'EXCLUSIVE_ACCESS': return <Star className="w-4 h-4" />;
  ...
  const progressPercentage = Math.round((campaign.currentMetrics.participants / campaign.targetMetrics.participants) * 100);
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <Avatar className="w-16 h-16">
                    ...
                    <AvatarFallback>{campaign.artist.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                  </Avatar>
                  <div>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="opacity-90">Join {campaign.currentMetrics.participants} other fans in this campaign!</p>
                      </div>
                      <Button 
                        variant="secondary" 
                        size="lg"
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-green-800">You're participating!</h3>
                          <p className="text-green-700">You have {campaign.userSubmissions.length} submissions</p>
                        </div>
                      </div>
                        ...
                        <DialogContent className="max-w-2xl">
                          ...
                          <div className="space-y-4">
                            ...
                            {submission.type !== 'text' && (
                              <div>
                                ...
                                <Input
                                  ...
                                  className="mt-1"
                                  accept={
                                    submission.type === 'image' ? 'image/*' :
                                    submission.type === 'video' ? 'video/*' :
                                    submission.type === 'audio' ? 'audio/*' : '*/*'
                                  }
                                  onChange={(e) => {
                            ...
                            <div className="flex justify-end gap-2">
                              <Button variant="outline" onClick={() => setShowParticipationModal(false)}>
                                Cancel
                              </Button>
                  ...
                  <CardContent>
                    <ul className="space-y-4">
                      {campaign.rewards.map((reward, index) => (
                        <li key={index} className="flex items-start gap-3 p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
              ...
              <TabsContent value="submissions" className="mt-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      See what other fans are sharing
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span>{campaign.currentMetrics.participants} / {campaign.targetMetrics.participants}</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">{campaign.currentMetrics.submissions}</div>
                      <div className="text-xs text-gray-600">Submissions</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-pink-600">{campaign.currentMetrics.engagement}</div>
