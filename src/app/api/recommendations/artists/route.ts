  ...
  return withApi(request, async (req) => {
    try {
      ...
      if (req.user.role === 'FAN') {
        ...
        const subscribedArtistIds = await prisma.subscription.findMany({
          where: {
            fanId: req.user.id,
            status: 'ACTIVE'
          },
        ...
        const userPreferences = await prisma.subscription.findMany({
          where: {
            fanId: req.user.id,
            status: 'ACTIVE'
          },
        ...
        recommendedArtists = await prisma.user.findMany({
          where: {
            role: 'ARTIST',
            id: {
      ...
      const formattedRecommendations = recommendedArtists.map(artist => {
        ...
        const genre = contentType === 'AUDIO' ? 'Music' : 
                     contentType === 'VIDEO' ? 'Video' :
                     contentType === 'IMAGE' ? 'Visual' : 'Content';

        // Generate description based on available data
        const subscriberCount = artist.artistProfile?.totalSubscribers || 0;
        const hasRecentContent = artist.content[0]?.createdAt && 
