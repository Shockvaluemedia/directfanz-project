...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user || user.role !== 'FAN') {
      return NextResponse.json(
        { error: 'Only fans can create subscriptions' },
        { status: 403 }
      );
    }
    ...
    if (amount < parseFloat(tier.minimumPrice.toString())) {
      return NextResponse.json(
        { error: 'Amount is below minimum price' },
        { status: 400 }
      );
    }

    // Check if artist is onboarded with Stripe
    if (!tier.artist.artistProfile?.stripeAccountId || !tier.artist.artistProfile.isStripeOnboarded) {
      return NextResponse.json(
    ...
    const stripeAccountId = tier.artist.artistProfile.stripeAccountId;
