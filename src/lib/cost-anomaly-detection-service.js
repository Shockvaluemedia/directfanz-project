...
class CostAnomalyDetectionService {
  ...
  updateServiceThresholds(thresholds) {
  ...
  async processAnomalies(anomalies) {
    ...
    for (const anomaly of anomalies) {
      ...
      const impact = parseFloat(anomaly.Impact?.TotalImpact || '0');
      ...
      const processedAnomaly = {
        ...
        service,
        impact: anomaly.Impact?.TotalImpact || '0',
  ...
  extractServiceFromAnomaly(anomaly) {
  ...
  shouldSkipAlert(anomalyId, service) {
  ...
  async sendAlertsForAnomalies(anomalies) {
    let alertsSent = 0;
...
module.exports = { CostAnomalyDetectionService };