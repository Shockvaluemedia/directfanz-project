...
function checkDependencies() {
  ...
  try {
    ...
    if (auditData.metadata.vulnerabilities.total > 0) {
      ...
      log(`⚠️  Found vulnerabilities: ${vulnCount.total} (${vulnCount.critical} critical, ${vulnCount.high} high, ${vulnCount.moderate} moderate, ${vulnCount.low} low)`, colors.yellow);
      ...
      if (auditData.vulnerabilities) {
        Object.values(auditData.vulnerabilities)
          ...
          .forEach(vuln => {
            log(`  - ${vuln.name}@${vuln.version}: ${vuln.severity} - ${vuln.title}`, colors.yellow);
          });
      }
...
function checkEnvFile() {
  ...
  if (!fs.existsSync('.env.example') && !fs.existsSync('.env.local.example')) {
    log('⚠️  No .env.example file found. Consider creating one with placeholder values.', colors.yellow);
    results.warnings.push({
      type: 'env_file',
...
function checkSecurityHeaders() {
  ...
  if (fs.existsSync(layoutFile)) {
    ...
    hasSecurityHeaders = content.includes('X-Content-Type-Options') || 
...
function checkGDPRCompliance() {
  ...
  const gdprFiles = [
    'src/components/ui/gdpr-consent.tsx',
    'src/app/api/user/gdpr',
  ];
...
function main() {
  ...
  log(`Scanned ${totalFiles} files, found ${issuesFound} potential issues`, issuesFound > 0 ? colors.yellow : colors.green);
  ...
  if (results.issues.length > 0) {
    log('\n❌ Security issues were found. Please address them before deploying to production.', colors.bold + colors.red);
    process.exit(1);
  } else if (results.warnings.length > 0) {
    log('\n⚠️  Security warnings were found. Consider addressing them to improve security.', colors.bold + colors.yellow);
    process.exit(0);
  } else {
