...
if (isMainThread) {
  ...
  console.log(
    `Starting load test with ${userCount} concurrent users for ${options.duration} seconds`
  );
  ...
  const intervalId = setInterval(() => {
    ...
    console.log(
      `Requests: ${stats.totalRequests} (${stats.successfulRequests} successful, ${stats.failedRequests} failed)`
    );
...
function printResults(stats, startTime) {
  ...
  console.log(
    `Successful requests: ${stats.successfulRequests} (${((stats.successfulRequests / stats.totalRequests) * 100).toFixed(2)}%)`
  );
  console.log(
    `Failed requests: ${stats.failedRequests} (${((stats.failedRequests / stats.totalRequests) * 100).toFixed(2)}%)`
  );

  if (stats.totalRequests > 0) {
    console.log(`Requests per second: ${(stats.totalRequests / testDuration).toFixed(2)}`);
    console.log(
      `Average response time: ${(stats.responseTimeTotal / stats.totalRequests).toFixed(2)}ms`
    );
