...
class SecurityTester {
  ...
  async testCSRFProtection() {
    ...
    try {
      ...
      if (csrfResponse.statusCode === 200) {
        ...
        this.recordTest('CSRF Token Generation', 'PASS', 
          `CSRF token generated successfully: ${this.session.csrfToken?.substring(0, 10)}...`);
      } else {
        this.recordTest('CSRF Token Generation', 'FAIL', 
      ...
      if (invalidRequest.statusCode === 403 || invalidRequest.statusCode === 401) {
        this.recordTest('CSRF Token Validation (Invalid)', 'PASS', 
          'Request properly rejected without CSRF token');
      } else {
        this.recordTest('CSRF Token Validation (Invalid)', 'FAIL', 
      ...
      if (validRequest.statusCode < 500) {
        this.recordTest('CSRF Token Validation (Valid)', 'PASS', 
          'Request properly accepted with valid CSRF token');
      } else {
        this.recordTest('CSRF Token Validation (Valid)', 'FAIL', 
    ...
    try {
      ...
      if (sessionCookie) {
        ...
        if (sessionCookie.sameSite) {
          securityScore++;
          this.recordTest('Session Cookie - SameSite', 'PASS', `SameSite: ${sessionCookie.sameSite}`);
        } else {
          issues.push('Missing SameSite attribute');
        }

        if (issues.length > 0) {
          this.recordTest('Session Cookie Security', 'WARNING', 
  ...
  async testRateLimiting() {
    ...
    try {
      ...
      if (rateLimited) {
        this.recordTest('Rate Limiting', 'PASS', 'Rate limiting is active (429 responses detected)');
      } else {
        this.recordTest('Rate Limiting', 'WARNING', 
          'No rate limiting detected - may need configuration');
      }

      const endTime = Date.now();
      const duration = endTime - startTime;
      this.recordTest('Rate Limiting Performance', 'INFO', 
  ...
  async testSecurityHeaders() {
    ...
    try {
      ...
      for (const [header, description] of Object.entries(expectedHeaders)) {
        if (headers[header]) {
          this.recordTest(`Security Header - ${header}`, 'PASS', 
            `${description}: ${headers[header]}`);
        } else {
          this.recordTest(`Security Header - ${header}`, 'WARNING', 
  ...
  async testAuthenticationFlows() {
    ...
    try {
      ...
      if (refreshResponse.statusCode === 401 || refreshResponse.statusCode === 403) {
        this.recordTest('Token Refresh - Unauthorized', 'PASS', 
          'Token refresh properly requires authentication');
      } else {
        this.recordTest('Token Refresh - Unauthorized', 'WARNING', 
  ...
  async testSQLInjection() {
    ...
    for (const payload of sqlPayloads) {
      try {
        const response = await this.makeRequest(`/api/auth/validate-session?userId=${encodeURIComponent(payload)}`);
        
        if (response.statusCode === 400 || response.statusCode === 422) {
          this.recordTest(`SQL Injection - Input Validation`, 'PASS', 
            'Malicious SQL payload properly rejected');
        } else if (response.statusCode === 500) {
          this.recordTest(`SQL Injection - Error Handling`, 'WARNING', 
    ...
    for (const payload of xssPayloads) {
      try {
        ...
        if (response.body.includes(payload)) {
          this.recordTest('XSS Protection', 'FAIL', 
            'XSS payload found unescaped in response');
        } else {
          this.recordTest('XSS Protection', 'PASS', 
            'XSS payload properly escaped or filtered');
        }
      } catch (error) {
  ...
  async testDOSProtection() {
    ...
    try {
      ...
      if (response.statusCode === 413) {
        this.recordTest('DoS Protection - Large Payload', 'PASS', 
          'Large payload properly rejected');
      } else if (duration > 5000) {
        this.recordTest('DoS Protection - Response Time', 'WARNING', 
