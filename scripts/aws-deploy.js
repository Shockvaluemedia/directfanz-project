...
async function checkPrerequisites() {
  ...
  try {
    ...
    log('Installation guide: https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html', colors.yellow);
    process.exit(1);
  }
  ...
  try {
    ...
    log('Installation guide: https://learn.hashicorp.com/tutorials/terraform/install-cli', colors.yellow);
    process.exit(1);
  }
...
async function setupTerraformBackend() {
  ...
  const createBucket = await question(`Create S3 bucket ${bucketName} for Terraform state? (y/n): `);
  if (createBucket.toLowerCase() === 'y') {
    executeCommand(`aws s3 mb s3://${bucketName} --region ${region}`, { ignoreError: true });
    
    // Enable versioning
    executeCommand(`aws s3api put-bucket-versioning --bucket ${bucketName} --versioning-configuration Status=Enabled`);
  ...
  const createTable = await question(`Create DynamoDB table ${tableName} for state locking? (y/n): `);
  if (createTable.toLowerCase() === 'y') {
    executeCommand(`aws dynamodb create-table \\
      --table-name ${tableName} \\
      --attribute-definitions AttributeName=LockID,AttributeType=S \\
      --key-schema AttributeName=LockID,KeyType=HASH \\
      --provisioned-throughput ReadCapacityUnits=5,WriteCapacityUnits=5 \\
      --region ${region}`, { ignoreError: true });
...
async function deployInfrastructure() {
  ...
  const environment = await question('Which environment do you want to deploy? (dev/staging/prod): ');
...
async function deployToECS() {
  ...
  const accountId = executeCommand('aws sts get-caller-identity --query Account --output text', { silent: true }).trim();
  const region = 'us-east-1';
  const ecrRepo = `${accountId}.dkr.ecr.${region}.amazonaws.com/direct-fan-platform`;
  
  // Create ECR repository if it doesn't exist
  executeCommand(`aws ecr create-repository --repository-name direct-fan-platform --region ${region}`, { ignoreError: true });
  
  // Get ECR login token
  executeCommand(`aws ecr get-login-password --region ${region} | docker login --username AWS --password-stdin ${ecrRepo}`);
  ...
  log('Consider using AWS Copilot for easier ECS deployments: https://aws.github.io/copilot-cli/', colors.yellow);
}
...
async function setupMonitoring() {
  ...
  executeCommand(`aws cloudwatch put-dashboard --dashboard-name "DirectFanPlatform" --dashboard-body file:///tmp/dashboard.json`);
