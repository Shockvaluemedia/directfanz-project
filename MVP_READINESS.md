...
- **Locked content teaser is partial.** The content page now renders a paywall
  for locked items (`GET /api/content/[id]` returns the item's metadata and
  tiers with `hasAccess: false` and no media URLs), but a fan browsing an
  artist's profile still sees only public content — gated items don't appear
  there as "🔒 subscribe to unlock" previews yet. Good next polish.
