  ...
  return {
    createCommentSchema: {
      parse: jest.fn((data) => {
        ...
        if (!data.contentId) throw new z.ZodError([{ path: ['contentId'], message: 'contentId is required', code: 'custom' }])
        if (!data.text || data.text.trim() === '') {
          throw new z.ZodError([{ path: ['text'], message: 'Comment cannot be empty', code: 'too_small' }])
        }
        return data
      }),
    },
