...
runner.test('API Health Check', async () => {
  ...
  if (response.status === 404) {
    ...
    runner.assert(response.status === 'healthy' || response.status === 'ok', 'Health check should return healthy status');
  }
});
...
runner.test('Campaign Discovery API', async () => {
  ...
  runner.assert(typeof response.pagination.total === 'number', 'Pagination should have total count');
...
runner.test('Campaign Discovery with Filters', async () => {
  ...
  runner.assert(Array.isArray(statusFilter.campaigns), 'Status filter should return campaigns array');
...
runner.test('Frontend Campaign Discovery Page', async () => {
  ...
  runner.assert(response.headers.get('content-type').includes('text/html'), 
    'Should return HTML content');
  
  const html = await response.text();
  runner.assert(html.includes('Discover Campaigns'), 
...
runner.test('Individual Campaign Pages', async () => {
  ...
  runner.assert(campaignApiResponse.id === firstCampaign.id, 
    'Individual campaign API should return correct campaign');
  runner.assert(campaignApiResponse.title, 
    'Campaign should have a title');
  runner.assert(campaignApiResponse.artist, 
    'Campaign should have artist information');
  
  // Test frontend page
  const frontendResponse = await fetch(`${testConfig.baseUrl}/campaigns/${firstCampaign.id}`);
  runner.assert(frontendResponse.ok, 
    'Individual campaign page should be accessible');
...
runner.test('Fan Dashboard APIs', async () => {
  ...
  runner.assert(fanCampaignsResponse.status === 401 || fanCampaignsResponse.error === 'Unauthorized', 
  ...
  runner.assert(fanStatsResponse.status === 401 || fanStatsResponse.error, 
...
runner.test('Fan Dashboard Frontend', async () => {
  ...
  runner.assert(response.status === 200 || response.status === 302 || response.status === 307, 
  ...
  try {
    ...
    if (campaignsResponse.campaigns.length > 0) {
      ...
      for (const field of requiredFields) {
        runner.assert(campaign[field] !== undefined, 
...
runner.test('API Error Handling', async () => {
  ...
  const is404 = invalidCampaignResponse.status === 404 || 
  ...
  runner.assert(malformedResponse.campaigns !== undefined, 
...
runner.test('Basic Performance Check', async () => {
  ...
  responses.forEach((response, index) => {
    runner.assert(Array.isArray(response.campaigns), 
  ...
  runner.assert(totalTime < 10000, 
    'API should handle concurrent requests within 10 seconds');
